    ...
    with open(req_path, "r") as f:
        for line in f:
            ...
            if line and not line.startswith('#'):
                ...
                if "==" not in line and ">=" not in line and "git+" not in line:
                    errors.append(f"Rule 13 Violation: Unpinned dependency detected: '{line}'.")
    return errors

def run_constitution_scan(base_dir=".", exclude=None):
    if exclude is None:
        exclude = ["venv", ".git", "__pycache__", "infrastructure"]
        
    total_errors = 0
    has_issues = False
    
    # 1. Dependency Check
    dep_errors = check_dependencies(base_dir)
    if dep_errors:
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for file in files:
            if file.endswith(".py") and file not in ["dev_constitution.py", "architecture_inspector.py"]:
...
if __name__ == "__main__":
    ...
    if not run_constitution_scan(args.dir):
        sys.exit(1)
    else:
